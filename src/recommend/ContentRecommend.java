...
public class ContentRecommend implements UtilConstant {
	...
	private void runCBF() {
		...
		try {
			bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(cbfcontentMatrix),"UTF-8"));
			// for all user	
	...
	private void readContentSimMatrix() {
		...
		String line;		
		try {
			br = new BufferedReader(new InputStreamReader(new FileInputStream(codeContentSimMatrix),"UTF-8"));
			int lineCount = 0;			
			while(true) {
				line = br.readLine();
				if(line == null)
